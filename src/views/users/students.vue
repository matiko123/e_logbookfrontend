...
<div v-if="!student.visitation_status && !user_permissions.includes('can_visit_students') && student.status ==='completed'">


 <div v-if="user_permissions.includes('can_view_students_competence')">
  <form v-if="studentCompetences.length == 0" @submit.prevent="submitCompetenceForm">
    ...
    <div class="modal-footer">
      <button type="button" class="btn btn-danger" data-bs-dismiss="modal" id="close">
      Close
    </button>
      <button type="submit" class="btn btn-primary">
  ...
  try {
    ...
    const fetchStudentCompetences  = async (id) => {
         ...
         then((response) => {
            ...
            console.log('student competences',studentCompetences.value);
        });
    };
